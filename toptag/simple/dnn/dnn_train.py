...
if(args.doEarlyStopping):
    ...
    mc = ModelCheckpoint(filepath =f'/users/yzhou276/work/toptag/simple/dnn/model/best_{dense_sizes}_dnn_pythia_{model_num}.keras', monitor='val_loss', mode='min', verbose=1, save_best_only=True)
    ...
    dnn_pythia_simple.fit(X_pythia_train.reshape(-1,X_pythia_train.shape[1]*X_pythia_train.shape[2]), Y_pythia_train,
                   ...
                   validation_data=(X_pythia_val.reshape(-1,X_pythia_val.shape[1]*X_pythia_val.shape[2]), Y_pythia_val),
                   verbose=1)
    dnn_pythia_simple.save(f'/users/yzhou276/work/toptag/simple/dnn/model/best_{dense_sizes}_dnn_pythia_{model_num}.keras')
...
if(args.doEarlyStopping):
    es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=patience)
    mc = ModelCheckpoint(filepath =f'/users/yzhou276/work/toptag/simple/dnn/model/best_{dense_sizes}_dnn_herwig_{model_num}.keras', monitor='val_loss', mode='min', verbose=1, save_best_only=True)
    ...
    dnn_herwig_simple.fit(X_herwig_train.reshape(-1,X_herwig_train.shape[1]*X_herwig_train.shape[2]), Y_herwig_train,
                   ...
                   validation_data=(X_herwig_val.reshape(-1,X_herwig_val.shape[1]*X_herwig_val.shape[2]), Y_herwig_val),
                   verbose=1)
    dnn_herwig_simple.save(f'/users/yzhou276/work/toptag/simple/dnn/model/best_{dense_sizes}_dnn_herwig_{model_num}.keras')
...
with open(f'/users/yzhou276/work/toptag/simple/dnn/auc/best_pythia_dnn_nlayers{nLayers}_dense{layerSize}_{model_num}.txt', 'w') as f:
...
with open(f'/users/yzhou276/work/toptag/simple/dnn/auc/best_herwig_dnn_nlayers{nLayers}_dense{layerSize}_{model_num}.txt', 'w') as f:
