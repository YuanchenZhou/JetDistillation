...
mix_latent_size_list = [16,32,64,128,256]
...
if use_scores:
    for i in pfn_teachers:
        ...
        '''
        mix_pred = i.predict(X_mix_test, batch_size=1000)
        mix_auc = roc_auc_score(Y_mix_test[:,1], mix_pred[:,1])
        mix_auc_scores.append(mix_auc)
else:
    for i in pfn_teachers:
        #pythia_auc_scores.append(1)
        #herwig_auc_scores.append(1)
        mix_auc_scores.append(1)

#print('Pythia AUC Scores:',pythia_auc_scores)
#print('Herwig AUC Scores:',herwig_auc_scores)
print('Mix AUC Scores:',mix_auc_scores)
############################################
...
X_train = X_mix_train
distiller_mix = MultiDistiller(student=dnn_mix_student, teachers=pfn_teachers, scores=mix_auc_scores)
...
if(args.doEarlyStopping):
    ...
    '''
    mc_d = ModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/mix_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers.keras',
                           ...
                           save_format="tf")
    '''
    mc_d = CustomModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/mix_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers.keras',
                                 monitor='val_categorical_crossentropy',
                                 mode='auto',
                                 verbose=1,
                                 save_best_only=True,
    ...
    #dnn_mix_student.save(f'/users/yzhou276/work/qgtag/student/dnn/avg_model/mix_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers.keras')
    ...
    distiller_mix.fit(X_mix_train,
              ...
              validation_data=(X_mix_val, Y_mix_val),#_for_distiller),
              verbose=1)
    dnn_mix_student.save(f'/users/yzhou276/work/qgtag/student/dnn/avg_model/mix_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teahcers)}mix_pfn_teachers.keras')
...
with open(f'/users/yzhou276/work/qgtag/student/dnn/avg_auc/best_mix_dnn_student_nlayers{nLayers}_dense{layerSize}_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers.txt', 'w') as f:
