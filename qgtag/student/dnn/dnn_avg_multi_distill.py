...
class MultiDistiller(tf.keras.Model):
    ...
    def train_step(
        ...
        weighted_teacher_predictions = tf.tensordot(tf.stack(teacher_predictions, axis=0), self.score_weights, axes=[[0], [0]])
        ...
        with tf.GradientTape() as tape:
            ...
            distillation_loss = self.distillation_loss_fn(
                tf.nn.softmax(weighted_teacher_predictions / self.temperature, axis=1),
...
if(args.doEarlyStopping):
    ...
    '''
    mc_d = ModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/pythia_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras',
                           ...
                           save_format="tf")
    '''
    mc_d = CustomModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/pythia_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras',
                                 monitor='val_categorical_crossentropy',
                                 mode='auto',
                                 verbose=1,
                                 save_best_only=True,
    ...
    #dnn_pythia_student.save(f'/users/yzhou276/work/qgtag/student/dnn/avg_model/pythia_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras')
...
if(args.doEarlyStopping):
    ...
    es_d = EarlyStopping(monitor='val_categorical_crossentropy', mode='auto', verbose=1, patience=args.patience)
    '''
    mc_d = ModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/herwig_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras',
                           monitor='val_categorical_crossentropy',
                           mode='auto',
                           verbose=1,
                           save_best_only=True,
                           save_format="tf")
    '''
    mc_d = CustomModelCheckpoint(filepath = f'/users/yzhou276/work/qgtag/student/dnn/avg_model/herwig_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras',
                                 monitor='val_categorical_crossentropy',
                                 mode='auto',
                                 verbose=1,
                                 save_best_only=True,
    ...
    #dnn_herwig_student.save(f'/users/yzhou276/work/qgtag/student/dnn/avg_model/herwig_student_{dense_sizes}_dnn_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.keras')
...
with open(f'/users/yzhou276/work/qgtag/student/dnn/avg_auc/best_pythia_dnn_student_nlayers{nLayers}_dense{layerSize}_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.txt', 'w') as f:
...
with open(f'/users/yzhou276/work/qgtag/student/dnn/avg_auc/best_herwig_dnn_student_nlayers{nLayers}_dense{layerSize}_by_{len(pythia_teachers)}pythia_{len(herwig_teachers)}herwig_{len(mix_teachers)}mix_pfn_teachers_{model_num}.txt', 'w') as f:
